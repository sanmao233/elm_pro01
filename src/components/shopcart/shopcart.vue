<template>
  ...
  <transition name="fade">
      <div class="list-mask" v-show="listShow" @click="hidehList"></div>
  </transition>
  </div>
</template>
<script type="text/ecmascript-6">
  ...
  export default {
    ...
    created () {
      this.$root.eventHub.$on('add-cart', this.drop)
    },
    beforeDestroy: function () {
      this.$root.eventHub.$off('add-cart', this.drop)
    },
    methods: {
      drop (target) {
  ...
  .shopcart
    ...
    .ball-container
      .ball
        ...
        bottom:22px
        z-index:200
        transition:transform .3s cubic-bezier(0.49,-0.29,0.75,0.41)
        .inner
          width:16px
          height:16px
          border-radius:50%
